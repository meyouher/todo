...
module.exports = async context => {
  // Skip this check if TODO is running as a GitHub Action
  if (!process.env.GITHUB_ACTION) {
    // Make a HEAD request to see the size of the diff without downloading it
    const headRequest = await getCommit(context, 'HEAD')
    const diffSize = headRequest.headers['content-length']

    // If its too large, thank u, next
